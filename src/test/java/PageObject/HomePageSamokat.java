...
public class HomePageSamokat {
    ...
    private final By accordionAnswerText = By.xpath(".//div[starts-with(@id, 'accordion__panel-')]");
    ; //Текст после клика по выпадающему списку
    ...
    public void clickAccordionHeader(int indexNumber) {
        driver.findElements(accordionHeader).get(indexNumber).click();
    }
    ...
    public void waitForLoadAccordionText(int index) {
        new WebDriverWait(driver, Duration.ofSeconds(3))
                .until(ExpectedConditions.visibilityOf(driver.findElements(accordionAnswerText).get(index)));
    }
}
